...
namespace TownApplication.IntegrationTests
{
    public class TownControllerIntegrationTests
    {
        ...
        public void AddTown_InvalidPopulation_ShouldThrowArgumentException(int invalidPopulation)
        {
            ...
            var exception = Assert.Throws<ArgumentException>(() => _controller.AddTown(validTown,invalidPopulation));
        ...
        public void AddTown_DuplicateTownName_DoesNotAddDuplicateTown()
        {
            ...
            _controller.AddTown(validTown, population+1);
            ...
            var items = result[0];
            Assert.Equal(population, items.Population);
            Assert.Equal(validTown, items.Name);
        }
        ...
        public void UpdateTown_ShouldUpdatePopulation()
        {
            ...
            var town = _controller.GetTownByName(validTown);
            ...
            var UpdatedTown = _controller.GetTownByName(validTown);
        ...
        public void DeleteTown_ShouldDeleteTown()
        {
            ...
            var deletedTown = _controller.GetTownByName(validTown);
        ...
        public void ListTowns_ShouldReturnTowns()
        {
            ...
            var validTown1 = "Sofia";
            var population1 = 1000;

            var validTown2 = "Paris";
            var population2 = 1500;

            // Add the towns to the database (pseudo-code, use your actual database interaction logic)
            //var townRepository = new TownRepository(); // Assuming you have a repository to interact with the database
            _controller.AddTown(validTown1, population1);
            _controller.AddTown(validTown2, population2);
            ...
            Assert.Equal(2, townsList.Count);

            // 2. Verify that each added town is present in the list.
            Assert.Contains(validTown1, townsList.Select(town => town.Name));
            Assert.Contains(validTown2, townsList.Select(town => town.Name));
        }
    }
}
